...
const Home = () => {
    ...
    const handlegetstarted = async () => {
        ...
        if (!address) {
            toaster.danger('Connect wallet first', { id: 'mess' })
        }

        hasLegacy(address).then((res) => console.log(res))

        if (await hasLegacy(address).then((res) => console.log(res))) {
            navigate('/profile');
        } else {
            ...
            <Box padding={{ base: '10px 40px', lg: "30px 80px" }} backgroundImage={imgBg} backgroundRepeat="no-repeat" backgroundSize="cover" h={{ base: '', lg: "100vh" }}>
                ...
                <Flex mt={{ base: '50px', lg: "100px" }} alignItems="center" display={{ base: 'block', lg: 'flex' }}>
                    <Box width={{ base: '100%', lg: "50%" }}>
                        ...
                        <CustomButton mt={{ base: '15px', lg: "30px" }} mb="20px" bg="brand.teal" color="brand.primary" hoverColor="brand.primary" isLoading={getStartedLoading} onClick={handlegetstarted}>Get Started</CustomButton>
                    </Box>
                </Flex>
            </Box>
            <main>
